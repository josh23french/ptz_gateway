...
// SerialIface implements the Iface interface for serial VISCA connections
type SerialIface struct {
	device       string
	port         *serial.Port
...
// NewSerialIface creates a new SerialIface
func NewSerialIface(device string) (*SerialIface, error) {
	return &SerialIface{
		device:       device,
		port:         nil,
		scanner:      nil,
...
// Start the interface
func (i *SerialIface) Start() error {
...
// Send a packet
func (i *SerialIface) Send(pkt *visca.Packet) error {
	if i.port == nil {
...
func (i *SerialIface) SetReceiveQueue(q chan *visca.Packet) {
	i.receiveQueue = q
	return
}
